...
/*setting up options menu*/
	...
	cal.gotoxy( 5,20);	cout<<"           88                                                          \n";
	cal.gotoxy( 5,21); 	cout<<",adPPYba,  88,dPPYba,   88  8b       d8  ,adPPYYba,  8 ,dPYba,,adPYba, \n";
	cal.gotoxy( 5,22); 	cout<<"SSS    sa  88P'     8a  88  `8b     d8'  aa     `Y8  88P'    88     8as\n";
	cal.gotoxy( 5,23); 	cout<<"   S8ba    88       88  88   `8b   d8'   ,adPPPPP88  88      88      88\n";
	cal.gotoxy( 5,24); 	cout<<"aa    DSD  88       88  88    `8b,d8'    88,    ,88  88      88      88\n";
	cal.gotoxy( 5,25); 	cout<<"SaYbbdPa'  88       88  88      s8s      `s8bbdPsY8  88      88      88\n";
	cal.gotoxy( 0, 0);
/***************************************************************************************************************************************/
	...
	if (kvt==5)
		{
			...
			string schedule;
			TCHAR upth[MAX_PATH];
			SHGetFolderPath(NULL,CSIDL_LOCAL_APPDATA|CSIDL_FLAG_CREATE,NULL, 0,upth);
			PathAppend(upth, TEXT("Calendar\\usrSchedule.cdb"));
			input.open(upth,ios::app);

				cal.docls();

				cal.setclr(15);	
				cal.gotoxy(30, 7);   	cout<<"\tINPUT Schedule";

				cal.gotoxy(28, 9);		cout<<"enter the year(YYYY)\n";
				cal.gotoxy(28,10);		cin >> year;
				cal.gotoxy(28,12);      cout<<"enter the month(mm)\n";
				cal.gotoxy(28,13);		cin >> month;
				cal.gotoxy(28,15);		cout<<"enter the date(dd)\n";
				cal.gotoxy(28,16);		cin >> date;
				cal.gotoxy(28,18);		cout<<"input your schedule\n";
				cal.gotoxy(28,19);		cin >> schedule;

				input <<"\t"<<usrn<<"\t"<< year<<"\t"<<month<<"\t"<<date<<"\t"<<schedule;
...
if (kvt==6)
	{
		...
		cal.docls();
		cal.setclr(15);	
		cal.gotoxy(30, 5);   	cout<<"\tOUTPUT Schedule";
		cal.gotoxy(24, 7);		cout<<"Enter the year[YYYY]\t";
		cin >> usryear;
		cal.gotoxy(24, 9);		cout<<"Enter the month[mm]\t";
		cin >> usrmon;
		int start;
		start = 12;
		...
		SHGetFolderPath(NULL,CSIDL_LOCAL_APPDATA|CSIDL_FLAG_CREATE,NULL, 0,upth);
		PathAppend(upth, TEXT("Calendar\\usrSchedule.cdb"));
		output.open(upth,ios::in);
		
		
		while(!output.eof()) {
			output>>usrname>>year>>month>>date>>sch;
			if(year == usryear){
				if(month == usrmon){
					if(strcmp(usrn,usrname) == 0){
						cal.gotoxy(28, start);		cout<<year<<". "<<month<<". "<<date<<". ";
						start++;
						cal.gotoxy(30, start);		cout<<sch;
						start+=2;
					}
				}
			}
